...
public class SitableEntity extends Entity {
      ...
      if (this.getPassengers().isEmpty() || this.level().isEmptyBlock(this.blockPosition())) {
        ...
        this.level()
            .updateNeighbourForOutputSignal(
                blockPosition(), this.level().getBlockState(blockPosition()).getBlock());
      }
    }
  }
  ...
  public static InteractionResult create(
      Level worldIn, BlockPos pos, double yOffset, Player player) {
    if (!worldIn.isClientSide()) {
      List<SitableEntity> seats =
  ...
  public Vec3 getDismountLocationForPassenger(LivingEntity entity) {
    ...
    Direction[] offsets = {
      original, original.getClockWise(), original.getCounterClockWise(), original.getOpposite()
    };
    for (Direction dir : offsets) {
      Vec3 safeVec =
          DismountHelper.findSafeDismountLocation(
              entity.getType(), this.level(), this.blockPosition().relative(dir), false);
      if (safeVec != null) {
